...
BATCH_XML="${BATCH_XML:-/data/output/mzmine_batch.xml}"
...
# Skip if mzXML files already exist (e.g. resuming after a failed run)
mkdir -p "${MZXML_DIR}"
if [ -z "$(ls -A "${MZXML_DIR}" 2>/dev/null)" ]; then
    ...
    echo "=== [stage_convert] mzXML files already present in ${MZXML_DIR}, skipping ==="
fi
